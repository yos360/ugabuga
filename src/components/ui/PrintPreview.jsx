...
export default function PrintPreview({title,children,onClose}){
  ...
  useEffect(()=>{stampQrCodes(root.current)},[children])
  ...
  return createPortal(<div ref={root}><dialog className="buga-print-dialog" ref={dialog} onCancel={onClose} aria-label={`תצוגה לפני הדפסה: ${title}`}><div className="buga-print-toolbar"><h2>{title}</h2><button onClick={onClose} aria-label="סגירת תצוגת ההדפסה">✕ חזרה</button><button onClick={print} disabled={busy}>{busy?'מכינים את הדף…':'🖨️ הדפסה / PDF'}</button></div><p className="buga-print-tip">A4 לאורך · דף נפרד לכל פריט. בחלון ההדפסה מומלץ לבטל כותרות עליונות ותחתונות.</p>{error&&<p role="alert">{error}</p>}<div className="buga-preview-pages">{children}</div></dialog><div id="buga-print-output" aria-hidden="true">{children}</div></div>,document.body)
}
