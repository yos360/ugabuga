...
const PRESETS={
 ...
 adults:{label:'🧠 למבוגרים',title:'תשבץ למבוגרים',words:[['תרבות','ספרים, מוזיקה ואמנות'],['זיכרון','מה שעוזר לנו לזכור'],['שיחה','מדברים בה יחד'],['חידה','שאלה שצריך לפתור'],['רעיון','מחשבה חדשה'],['חברות','קשר טוב בין אנשים']]},
 tashbetz:{label:'📰 תשחץ קלאסי',title:'תשחץ קלאסי',words:[['ישראל','המדינה שלנו'],['ירושלים','עיר הבירה'],['אביב','עונה אחרי החורף'],['ספר','קוראים בו'],['מוזיקה','שומעים אותה'],['שמש','מאירה ביום']]},
}
const SIZE=15
function makeCrossword(entries){
 ...
 const numbers={}; let next=1
 placed.forEach(p=>{const key=`${p.row},${p.col}`;if(numbers[key]==null)numbers[key]=next++})
 return {grid,placed,numbers}
}
