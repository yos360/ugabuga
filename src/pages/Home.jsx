...
export default function Home() {
  ...
  return <><SEO path="/" /><div className="home-v2">
    ...
    <section className="home-extra"><h2>משחקי BUGA המרכזיים</h2><div className="home-shortcuts">{[['🎭 אמת או בוגה','/tools/truth-or-buga'],['🗺️ ארץ־עיר','/tools/eretz-ir'],['🎟️ בינגו','/tools/bingo-maker'],['יום הולדת','/games/birthday'],['בלי ציוד','/games/no-equipment'],['משפחה','/games/family']].map(([label,to])=><Link key={to} to={to}>{label} ←</Link>)}</div><div className="home-extras"><Link to="/tools/truth-or-buga"><h3>🎭 אמת או בוגה</h3><p>אמת או שקר, לבד או תחרות קבוצות.</p></Link><Link to="/tools/eretz-ir"><h3>🗺️ ארץ־עיר</h3><p>אות אקראית, טיימר וניקוד.</p></Link><Link to="/tools/bingo-maker"><h3>🎟️ בינגו</h3><p>כרטיסיות מוכנות ומותאמות להדפסה.</p></Link></div></section>
  </div></>
}
