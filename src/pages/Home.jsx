...
export default function Home() {
  ...
  return <><SEO path="/" /><div className="home-v2">
    <section className="home-intro"><h1>מה בא לכם לעשות?</h1><p>משחקים, יצירה, לימוד וחוויות לכולם — בכל גיל ובכל הזדמנות.</p></section>
    ...
    <section className="home-extra"><h2>לכל מפגש יש משחק</h2><div className="home-shortcuts">{[['יום הולדת','/games/birthday'],['בלי ציוד','/games/no-equipment'],['5 דקות','/games/5-minutes'],['משפחה','/games/family'],['צהרון','/games/afterschool'],['בוגה טאון','/tools/buga-town'],['בינגו משלכם','/tools/bingo-maker'],['חידות','/tools/riddles']].map(([label,to])=><Link key={to} to={to}>{label} ←</Link>)}</div><div className="home-extras"><Link to="/calculator"><h3>🎉 מחשבון מסיבה</h3><p>מתכננים כמויות ומכינים רשימת קניות.</p></Link><Link to="/birthday-songs"><h3>🎵 שירי יום הולדת</h3><p>מפעילים מוזיקה ומתחילים לחגוג.</p></Link><Link to="/blog"><h3>חדשות הבוגה</h3><p>רעיונות ופעילויות למפגש הבא שלכם.</p></Link></div></section>
  </div></>
}
