...
import { suppliersDb, supplierAuth, currentSupplierUser, signInWithGoogle, signInWithEmail, signOutSupplier, waLink, OWNER_WHATSAPP } from '../../utils/suppliersDb'
...
function SignIn() {
  ...
  const google = async () => { setErr(''); const { error } = await signInWithGoogle(); if (error) setErr('ההתחברות עם גוגל עוד לא זמינה. נסו עם קישור למייל.') }
  const mail = async e => {
  ...
  return <div className="mx-auto max-w-md space-y-4">
    <button onClick={google} className="flex min-h-[56px] w-full items-center justify-center gap-3 rounded-2xl border-2 border-slate-800 bg-white text-lg font-bold">
